...
export function generatePassword(options: PasswordGeneratorOptions): string {
  ...
  if (options.includeLowercase) pools.push(filterAmbiguous(CHARSETS.lowercase, options));
  if (options.includeUppercase) pools.push(filterAmbiguous(CHARSETS.uppercase, options));
  if (options.includeNumbers) pools.push(filterAmbiguous(CHARSETS.numbers, options));
  if (options.includeSymbols) pools.push(filterAmbiguous(CHARSETS.symbols, options));
...
function filterAmbiguous(charset: string, options: PasswordGeneratorOptions): string {
  if (!options.excludeAmbiguous) return charset;
